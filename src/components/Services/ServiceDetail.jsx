...
export default function ServiceDetail({ service, canEdit, isPastor, onRefresh }) {
  ...
  const fetchServiceSongs = async () => {
    const { data } = await supabase.from('service_songs').select('*, songs(*)').eq('service_id', service.id).order('order_index')
    setSongs(data || [])
  }
  ...
  const fetchChat = async () => {
    const { data } = await supabase.from('service_chat').select('*, profiles(full_name)').eq('service_id', service.id).order('created_at')
    setChatMessages(data || [])
  }

  const fetchComments = async () => {
    const { data } = await supabase.from('comments').select('*, profiles(full_name)').eq('service_id', service.id).order('created_at')
    setComments(data || [])
  }

  const addSong = async (songId) => {
    await supabase.from('service_songs').insert({ service_id: service.id, song_id: songId, order_index: songs.length })
  ...
  const handleDragEnd = async (event) => {
    ...
    await Promise.all(newSongs.map((s, i) => supabase.from('service_songs').update({ order_index: i }).eq('id', s.id)))
  }

  const sendMessage = async () => {
    if (!newMessage.trim() || sendingMsg) return
    setSendingMsg(true)
    await supabase.from('service_chat').insert({ service_id: service.id, user_id: user.id, message: newMessage.trim() })
  ...
  const addComment = async () => {
    ...
    await supabase.from('comments').insert({ service_id: service.id, user_id: user.id, content: newComment.trim() })
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(0,212,255,0.08), rgba(124,58,237,0.06))', borderBottom: '1px solid rgba(0,212,255,0.1)', padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {service.location && <p style={{ color: '#64748b', fontSize: '11px', margin: '0 0 4px' }}>📍 {service.location}</p>}
            {service.description && (
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        {view === 'songs' && (
          <div>
            ...
            {activeSong && (
              <SongViewer key={activeSong.id} song={activeSong} autoExpand={isMobile}
                hasNext={hasNext} hasPrev={hasPrev} onNext={goNext} onPrev={goPrev}

        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ color: '#f59e0b', fontSize: '12px', margin: '0 0 2px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              📅 {dayjs.utc(selected.date).local().format('DD/MM/YYYY HH:mm')}
            </p>
            {selected.location && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ margin: '0 0 2px', color: '#f59e0b', fontSize: '12px' }}>
            📅 {dayjs.utc(rehearsal.date).local().format('DD/MM/YYYY HH:mm')}
          </p>
          {rehearsal.location && (
...
function RehearsalForm({ rehearsal, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    date:        rehearsal?.date ? dayjs.utc(rehearsal.date).local().format('YYYY-MM-DDTHH:mm') : '',
  ...
  const handleSubmit = async e => {
    ...
    const formToSave = {
      ...form,
      date: form.date ? dayjs(form.date).utc().format() : null
    }
    if (rehearsal) {
      await supabase.from('rehearsals').update({ ...formToSave, updated_at: new Date() }).eq('id', rehearsal.id)
    } else {
      await supabase.from('rehearsals').insert({ ...formToSave, created_by: user.id })
    }
    setSaving(false)
    onSaved()
  }

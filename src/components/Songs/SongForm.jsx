      ...
      <div style={{ background: '#0d1b2a', border: '1px solid rgba(0,212,255,0.3)', borderRadius: '16px', width: '100%', maxWidth: '580px', boxShadow: '0 0 60px rgba(0,212,255,0.1)', animation: 'fadeInUp 0.3s ease forwards', margin: 'auto', overflow: 'hidden' }}>
        <div style={{ padding: '18px 20px' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            {activeSection === 'info' && (
              <>
                <div><label style={labelStyle}>Nombre *</label><input value={form.title} onChange={e => set('title', e.target.value)} required className="input-field" placeholder="Nombre de la canción" /></div>
                ...
                <div>
                  ...
                  <input value={form.youtube_url} onChange={e => set('youtube_url', e.target.value)} className="input-field" placeholder="https://youtube.com/watch?v=..." type="url" />
                </div>
              </>
            )}
            ...
            {activeSection === 'chords' && (
              <div>
                <label style={labelStyle}>
                  Acordes con letra
                  <span style={{ color: '#475569', fontWeight: 'normal', marginLeft: '6px', fontSize: '10px' }}>usa [Verso 1] [Coro] para secciones</span>
                </label>
                ...
                <textarea value={form.chords} onChange={e => set('chords', e.target.value)} rows={12} className="input-field" style={{ fontFamily: 'monospace', resize: 'vertical', fontSize: '13px', lineHeight: '1.7' }} placeholder={`[Verso 1]\nG          Em\nSanto, santo es el Señor`} />
              </div>
            )}
            ...
            {activeSection === 'lyrics' && (
              <div>
                ...
                <textarea value={form.lyrics} onChange={e => set('lyrics', e.target.value)} rows={14} className="input-field" style={{ resize: 'vertical', fontSize: '14px', lineHeight: '1.8' }} placeholder={`[Verso 1]\nSanto, santo es el Señor\n\n[Coro]\nDigno es el Señor`} />
              </div>
            )}

            <div style={{ display: 'flex', gap: '10px', paddingTop: '4px' }}>
              <button type="button" onClick={onClose} style={{ flex: 1, padding: '11px', borderRadius: '8px', cursor: 'pointer', background: 'transparent', border: '1px solid rgba(100,116,139,0.4)', color: '#94a3b8', fontSize: '13px', fontWeight: '600', letterSpacing: '1px' }}>CANCELAR</button>

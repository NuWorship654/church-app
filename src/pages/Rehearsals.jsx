...
export default function Rehearsals() {
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
          ...
          {canEdit && (
            <button onClick={() => { setEditing(selected); setShowForm(true) }} style={{
              ...
              color: '#f59e0b', fontSize: '11px', fontWeight: '600', flexShrink: 0
            }}>✎ EDITAR</button>
          )}
        </div>
        {selected.description && (
          <p style={{ color: '#94a3b8', fontSize: '12px', margin: '10px 0 0', padding: '8px 12px', borderRadius: '8px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)', wordBreak: 'break-word' }}>{selected.description}</p>
        )}
      </div>

      <div style={{ padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
          <p style={{ color: '#f59e0b', fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', margin: 0 }}>
          ...
          {canEdit && (
            <button onClick={() => { setShowAddSong(!showAddSong); setSearchSong('') }} style={{
              ...
              color: showAddSong ? '#06ffa5' : '#f59e0b', fontSize: '11px', fontWeight: '700'
            }}>{showAddSong ? '✕ CERRAR' : '+ AGREGAR'}</button>
          )}
        </div>

        {showAddSong && (
          <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(6,255,165,0.2)', borderRadius: '10px', padding: '12px', marginBottom: '12px' }}>
            <input type="text" placeholder="Buscar canción..." value={searchSong} onChange={e => setSearchSong(e.target.value)}
          ...
          <div style={{ display: 'flex', gap: '5px', marginBottom: '12px', overflowX: 'auto', paddingBottom: '4px' }}>
            {songs.map((ss, i) => (
              <button key={ss.id} onClick={() => setActiveSongIndex(i)} style={{
                flexShrink: 0, padding: '7px 10px', borderRadius: '8px', cursor: 'pointer',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <span style={{ width: '16px', height: '16px', borderRadius: '50%', flexShrink: 0, background: activeSongIndex === i ? 'rgba(245,158,11,0.3)' : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', color: activeSongIndex === i ? '#f59e0b' : '#64748b' }}>{i + 1}</span>
                  ...
                  {canEdit && (
                    <span onClick={e => { e.stopPropagation(); removeSong(ss.id, i) }}
                      style={{ color: '#475569', cursor: 'pointer', fontSize: '11px', padding: '2px 4px' }}
        ...
        {songs.length === 0 && !showAddSong && (
          <div style={{ textAlign: 'center', padding: '28px', color: '#475569' }}>
            ...
            {canEdit && (
              <button onClick={() => setShowAddSong(true)} style={{ padding: '7px 18px', borderRadius: '8px', cursor: 'pointer', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', color: '#f59e0b', fontSize: '12px', fontWeight: '600' }}>
                + AGREGAR CANCIÓN
              </button>
            )}
          </div>
        ...
        {activeSong && songs.length > 0 && (
          <SongViewer key={activeSong.id} song={activeSong} autoExpand={isMobile} hasNext={hasNext} hasPrev={hasPrev} onNext={goNext} onPrev={goPrev} serviceSongs={songs.map(ss => ss.songs)} />
        )}
      </div>
    </div>
  )
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.5s ease forwards', width: '100%', overflowX: 'hidden' }}>
      ...
      {showDetail && selected ? (
        <div>
          <button onClick={() => setShowDetail(false)} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px', background: 'none', border: 'none', color: '#f59e0b', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>
            ← VOLVER
          </button>
          <DetailPanel />
        </div>
      ) : (
        <div className="rehearsals-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '16px' }}>
              ...
              <>
                {upcoming.length > 0 && (
                  <>
                    ...
                    {upcoming.map((r, i) => <RehearsalCard key={r.id} rehearsal={r} selected={selected} onSelect={selectRehearsal} canEdit={canEdit} onEdit={() => { setEditing(r); setShowForm(true) }} onDelete={() => handleDelete(r.id)} index={i} />)}
                  </>
                )}
                {past.length > 0 && (
                  <>
                    <p style={{ color: '#64748b', fontSize: '10px', letterSpacing: '2px', margin: '14px 0 8px', textTransform: 'uppercase' }}>ANTERIORES</p>
                    {past.map((r, i) => <RehearsalCard key={r.id} rehearsal={r} selected={selected} onSelect={selectRehearsal} canEdit={canEdit} onEdit={() => { setEditing(r); setShowForm(true) }} onDelete={() => handleDelete(r.id)} index={i} past />)}
                  </>
                )}
              </>
            )}
          ...
          <div className="rehearsals-detail-desktop" style={{ minWidth: 0 }}>
            {selected ? <DetailPanel /> : (
              <div style={{ background: 'rgba(13,27,42,0.5)', border: '1px dashed rgba(245,158,11,0.2)', borderRadius: '12px', padding: '60px 20px', textAlign: 'center', color: '#64748b' }}>
                <div style={{ fontSize: '36px', marginBottom: '12px', opacity: 0.3 }}>🎸</div>
                <p style={{ margin: 0, fontSize: '14px' }}>Selecciona un ensayo</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <p style={{ margin: '0 0 3px', fontWeight: '600', color: past ? '#94a3b8' : '#e2e8f0', fontSize: '13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{rehearsal.title}</p>
...
function RehearsalForm({ rehearsal, onClose, onSaved }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 50, padding: '16px', backdropFilter: 'blur(6px)', overflowY: 'auto' }}>
      <div style={{ background: 'linear-gradient(135deg, #0d1b2a 0%, #0a1628 100%)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: '20px', width: '100%', maxWidth: '500px', animation: 'fadeInUp 0.3s ease forwards', margin: 'auto', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.1), rgba(6,255,165,0.08))', borderBottom: '1px solid rgba(245,158,11,0.15)', padding: '18px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '14px', color: '#f59e0b', margin: 0 }}>
        ...
        <div style={{ padding: '20px' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div><label style={labelStyle}>Título *</label><input value={form.title} onChange={e => set('title', e.target.value)} required className="input-field" placeholder="Ej: Ensayo dominical" /></div>
            ...
            <div><label style={labelStyle}>📍 Lugar</label><input value={form.location} onChange={e => set('location', e.target.value)} className="input-field" placeholder="Ej: Salón de música" /></div>
            <div><label style={labelStyle}>📝 Descripción</label><textarea value={form.description} onChange={e => set('description', e.target.value)} rows={3} className="input-field" style={{ resize: 'vertical' }} placeholder="Notas del ensayo..." /></div>
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button type="submit" disabled={saving} style={{ flex: 2, padding: '11px', borderRadius: '10px', cursor: saving ? 'not-allowed' : 'pointer', background: saving ? 'rgba(245,158,11,0.1)' : 'linear-gradient(135deg, #f59e0b, #06ffa5)', border: 'none', color: saving ? '#f59e0b' : '#0d1b2a', fontSize: '13px', fontWeight: '700' }}>

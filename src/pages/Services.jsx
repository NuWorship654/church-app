    ...
    <div style={{ animation: 'fadeInUp 0.5s ease forwards', width: '100%', overflowX: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', gap: '8px', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
          ...
          <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '20px', background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.2)', color: '#00d4ff', flexShrink: 0 }}>{services.length}</span>
        </div>
      ...
      {view === 'calendar' && (
        <div style={{ background: 'rgba(13,27,42,0.9)', border: '1px solid rgba(0,212,255,0.12)', borderRadius: '14px', overflow: 'hidden', marginBottom: '14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid rgba(0,212,255,0.08)', background: 'rgba(0,212,255,0.04)' }}>
            ...
            <p style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '12px', color: '#e2e8f0', margin: 0, textTransform: 'uppercase', letterSpacing: '2px' }}>{calendarMonth.format('MMMM YYYY')}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px', padding: '4px 10px 14px' }}>
            {calDays.map((day, i) => {
              ...
              return (
                <div key={i} style={{ minHeight: '44px', borderRadius: '7px', padding: '3px', background: isToday ? 'rgba(0,212,255,0.1)' : hasService ? 'rgba(6,255,165,0.06)' : 'transparent', border: isToday ? '1px solid rgba(0,212,255,0.4)' : hasService ? '1px solid rgba(6,255,165,0.25)' : '1px solid transparent', cursor: hasService ? 'pointer' : 'default', transition: 'all 0.15s', overflow: 'hidden' }}
          ...
          <div style={{ width: '100%' }}>
            {loading ? (
              ...
              <>
                ...
                {past.length > 0 && (
                  <div>
                    <p style={{ color: '#64748b', fontSize: '10px', letterSpacing: '2px', margin: '0 0 8px', textTransform: 'uppercase' }}>ANTERIORES — {past.length}</p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                      {past.map((s, i) => (
                        <ServiceCard key={s.id} service={s} selected={selected}
                          onSelect={sv => { setSelected(sv); setShowDetail(true) }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', paddingLeft: past ? 0 : '6px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px', flexWrap: 'wrap' }}>
            <p style={{ margin: 0, fontWeight: '600', color: past ? '#94a3b8' : '#e2e8f0', fontSize: '14px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{service.title}</p>
            ...
            {isSoon && !isToday && <span style={{ fontSize: '8px', padding: '1px 6px', borderRadius: '20px', background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)', color: '#f59e0b', fontWeight: '700', flexShrink: 0 }}>PRONTO</span>}
          </div>
          <p style={{ margin: '0 0 1px', color: '#00d4ff', fontSize: '11px', textTransform: 'capitalize' }}>📅 {dayjs(service.date).format('ddd DD MMM · HH:mm')}</p>

...
export default function Songs() {
  ...
  const fetchFavorites = async () => {
    ...
    const { data } = await supabase
      .from('user_favorites').select('song_id').eq('user_id', user.id)
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.5s ease forwards' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <input
          type="text" placeholder="Buscar cancion..."
        ...
        }}>{filterFav ? '★ Favoritas' : '☆ Favoritas'}</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '20px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '70vh', overflowY: 'auto', paddingRight: '4px' }}>
          {loading ? (
            ...
            filtered.map((song, i) => (
              <div key={song.id} onClick={() => setSelected(song)} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                      ...
                      {song.bpm > 0 && (
                        <span style={{ fontSize: '10px', color: '#06ffa5' }}>
                          {'♩ ' + song.bpm}
                        </span>
                      )}
                    </div>

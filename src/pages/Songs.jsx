  ...
  return (
    <div style={{ animation: 'fadeInUp 0.5s ease forwards', width: '100%', overflowX: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', gap: '8px', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
          ...
          <span style={{
            ...
            background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.2)', color: '#00d4ff', flexShrink: 0
          ...
          {canEdit && (
            <>
              <button onClick={() => setImportMode(!importMode)} style={{
                ...
                fontSize: '11px', fontWeight: '600', letterSpacing: '0.5px', whiteSpace: 'nowrap'
              }}>IMPORTAR</button>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => { setImportMode(false); setImportText('') }} style={{
              ...
              background: 'transparent', border: '1px solid rgba(100,116,139,0.4)', color: '#94a3b8', fontSize: '13px'
            }}>Cancelar</button>
      ...
      {showViewer && selected ? (
        ...
        <div className="songs-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '16px' }}>
                ...
                {filtered.map(song => (
                  <div key={song.id} onClick={() => { setSelected(song); setShowViewer(true) }} style={{
                    ...
                    <div style={{
                      ...
                      fontSize: '10px', fontWeight: '700', color: '#00d4ff', fontFamily: 'Orbitron, sans-serif'
                    }}>{song.original_key || '?'}</div>
                    <p style={{ margin: '0 0 3px', fontSize: '11px', fontWeight: '600', color: '#e2e8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {song.title}
                    </p>
                  ...
                  Object.keys(grouped).sort().map(letter => (
                    <div key={letter}>
                      <div style={{
                        ...
                        background: 'rgba(2,8,23,0.95)', backdropFilter: 'blur(4px)',
          ...
          {/* Visor desktop */}
          <div className="songs-viewer-desktop" style={{ minWidth: 0 }}>
            {selected ? (
              <SongViewer song={selected} hasNext={hasNext} hasPrev={hasPrev} onNext={goNext} onPrev={goPrev} serviceSongs={filtered} />
            ) : (
              <div style={{
                ...
                <p style={{ margin: '0 0 6px', fontSize: '14px' }}>Selecciona una canción</p>
                <p style={{ margin: 0, fontSize: '11px', color: '#334155' }}>{filtered.length} disponible{filtered.length !== 1 ? 's' : ''}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {showForm && (
        <SongForm song={editing} onClose={() => setShowForm(false)} onSaved={() => { fetchSongs(); setShowForm(false) }} />
      )}
    </div>
  )
}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
        <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '8px', fontWeight: '700', color: isSelected ? '#00d4ff' : '#a78bfa',
            fontFamily: 'Orbitron, sans-serif'
          }}>{song.original_key || '?'}</div>
          <div style={{ minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              {favorites.includes(song.id) && <span style={{ color: '#f59e0b', fontSize: '10px', flexShrink: 0 }}>★</span>}
              <p style={{ margin: 0, fontWeight: '600', color: isSelected ? '#ffffff' : '#e2e8f0', fontSize: '13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {song.title}
              </p>
            </div>
        ...
        {canEdit && (
          <div style={{ display: 'flex', gap: '2px', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
            <button onClick={onEdit} style={{ background: 'none', border: 'none', color: '#00d4ff', cursor: 'pointer', fontSize: '12px', padding: '4px 6px', borderRadius: '5px' }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(0,212,255,0.1)'}
              onMouseLeave={e => e.currentTarget.style.background = 'none'}>✎</button>
            <button onClick={onDelete} style={{ background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', fontSize: '12px', padding: '4px 6px', borderRadius: '5px' }}
